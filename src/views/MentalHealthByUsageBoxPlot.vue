<template>
  <div>
    <h1>Mental Health by Usage Box Plot</h1>
    ...
    <div class="mt-4">
      <img
        ...
        alt="Mental Health by Usage Box Plot"
    ...
    <button @click="fetchBoxplot" class="mt-4 bg-blue-500 text-white px-4 py-2 rounded">
      Generar Box Plot
    </button>
  </div>
</template>
...
onMounted(() => {
  // Cargar datos almacenados al montar la vista
...
// Actualizar el resultado cuando cambie la predicción
...
const updatePredictionResult = () => {
  if (predictionStore.prediction) {
    predictionResult.value = `Predicción Mental: ${predictionStore.prediction.mental_health_score || 'N/A'}`;
...
const fetchBoxplot = async () => {
  try {
    await predictionStore.getMentalHealthByUsageBoxplot();
    ...
    predictionResult.value = `Error al obtener el boxplot: ${errorMessage}`;
    imageError.value = errorMessage;
    console.error('Error en fetchBoxplot:', error);
  }
};
